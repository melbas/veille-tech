  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-5 py-10">
        {latest ? (
          <Link href={`/digest/${latest.date}`}
                className="block rounded-2xl border border-white/10 bg-gradient-to-br from-violet-500/10 to-fuchsia-500/5 p-6 mb-12 hover:border-white/25 transition">
          ...
          <div className="rounded-2xl border border-white/10 p-6 mb-12 text-white/50">
        ...
        {all.length <= 0 ? (
          ...
          <ul className="space-y-1">
            {all.map((d) => (
              <li key={d.date}>
                <Link href={`/digest/${d.date}`}
                      className="flex items-baseline justify-between gap-4 py-2.5 px-3 -mx-3 rounded-lg hover:bg-white/5 transition group">
